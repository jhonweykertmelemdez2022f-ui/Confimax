...
-- Búsqueda de productos por nombre (búsqueda parcial con LIKE)
...
-- Búsqueda por nombre (autocompletar con LIKE)
CREATE INDEX IF NOT EXISTS idx_customers_name_pattern 
ON customers(first_name text_pattern_ops, last_name text_pattern_ops) 
WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_credits_sync_pull 
ON credits(updated_at, version) 
WHERE deleted_at IS NULL;
